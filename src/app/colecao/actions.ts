...
export async function atualizarFigurinha(
  ...
  if (
    ...
    return {
      ok: false,
      error:
        "A figurinha informada é inválida.",
    };
  }
  ...
  /*
   * Quantidade zero remove a figurinha
  ...
  if (quantidade === 0) {
    ...
    /*
     * Não usamos revalidatePath aqui.
     * A tela já foi atualizada localmente.
     */
    return {
      ok: true,
    };
   ...
   * Sem revalidatePath.
   *
   * Isso evita recarregar as 980
   * figurinhas depois de cada toque.
   */
  return {
    ok: true,
  };
